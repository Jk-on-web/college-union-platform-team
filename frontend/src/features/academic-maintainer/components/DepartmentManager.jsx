 ...
 * DepartmentManager
 * ─────────────────
 * Admin-only component for managing academic departments.
 * Shown as the "Departments" tab inside AcademicMaintainerPage.
 *
 * Sections:
 *  1. Summary stat-bar
 *  2. Departments table  – with Edit Name / Toggle Active / Delete per row
 *  3. Pending student requests table – Approve / Reject
 *  4. Add Department modal  (inline, no routing needed)
 *  5. Edit Name modal
 */
...
function AddDepartmentModal({ onClose, onCreated, notify }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        notify(`Department "${name.trim()}" created ✓`);
        onCreated(res.data);
        onClose();
      } else {
...
// ── Approve request modal (asks for optional note) ────────────────────────────
function ApproveRequestModal({ request, onClose, onApproved, notify }) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (res.ok) {
        notify(`"${request.name}" approved and created ✓`);
...
export default function DepartmentManager({ notify }) {
  ...
  const [reqBusy,  setReqBusy]  = useState({}); // per-row busy for toggle/delete

  // Modal state
  const [addOpen,    setAddOpen]    = useState(false);
  const [editTarget, setEditTarget] = useState(null);  // dept object
  const [approveReq, setApproveReq] = useState(null);  // request object
  const [rejectReq,  setRejectReq]  = useState(null);  // request object
  ...
  const handleDelete = async (dept) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        notify(`"${dept.name}" deleted.`);
      } else {
        notify(res.error || "Delete failed.");
      }
  ...
  const handleApproved = (reqId) => {
    ...
    // Refresh dept list so newly-created dept shows up
    academicsService.adminGetAllDepartments().then((r) => { if (r.ok) setDepts(r.data || []); });
  };
  ...
  // Derived counts
  const totalActive   = depts.filter((d) =>  d.active).length;
  const totalInactive = depts.filter((d) => !d.active).length;
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px", marginBottom: "18px" }}>
        {[
          { label: "Total Departments", value: depts.length, icon: <Building2 size={16} /> },
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
              ...
              <tbody>
                {depts.map((dept) => {
                  ...
                  return (
                    <tr key={dept.id} style={{ background: "#fff" }}
